...
type PriceResponse = { gold: Market; coin: Market; usd: Market; usdt: Market; goldOunce: number | null; updatedAt: string; refreshAfterSeconds: number };
type QuoteTab = "usd" | "usdt";
type GoldTab = "gold" | "coin";
...
const usdFallback: Market = { averagePrice: null, sources: [
  ...
  { id: "usd-shahrexchange-exchange", name: "صرافی بانک شهر", price: null, url: "https://www.tgju.org/currency-exchange/34509/shahrexchange-exchange", status: "unavailable", updatedAt: null, domain: "tgju.org", logoUrl: "https://platform.tgju.org/files/images/shahrexchange-1649663343.png" },
...
const coinFallback: Market = { averagePrice: null, sources: [
  { id: "coin-sekee", name: "سکه امامی", price: null, url: "https://www.tgju.org/profile/sekee", status: "unavailable", updatedAt: null, domain: "tgju.org" },
  { id: "coin-sekeb", name: "سکه بهار آزادی", price: null, url: "https://www.tgju.org/profile/sekeb", status: "unavailable", updatedAt: null, domain: "tgju.org" },
  { id: "coin-nim", name: "نیم سکه", price: null, url: "https://www.tgju.org/profile/nim", status: "unavailable", updatedAt: null, domain: "tgju.org" },
  { id: "coin-rob", name: "ربع سکه", price: null, url: "https://www.tgju.org/profile/rob", status: "unavailable", updatedAt: null, domain: "tgju.org" },
  { id: "coin-gerami", name: "سکه گرمی", price: null, url: "https://www.tgju.org/profile/gerami", status: "unavailable", updatedAt: null, domain: "tgju.org" },
] };
const fallback: PriceResponse = { gold: goldFallback, coin: coinFallback, usd: usdFallback, usdt: tetherFallback, goldOunce: null, updatedAt: "", refreshAfterSeconds: REFRESH_SECONDS };
...
function updateSource(current: PriceResponse | null, marketName: GoldTab | QuoteTab, nextSource: PriceSource, updatedAt: string): PriceResponse {
...
function MarketPanel({ market, title, titleEnglish, unit, tone, summaryLabel = "قیمت میانگین", summaryValue, tabs, activeTab, onTabChange, goldTabs, activeGoldTab, onGoldTabChange, intrinsicGoldPrice, sortSources = true }: { market: Market; title: string; titleEnglish: string; unit: string; tone: "gold" | "dollar" | "coin"; summaryLabel?: string; summaryValue?: string; tabs?: boolean; activeTab?: QuoteTab; onTabChange?: (tab: QuoteTab) => void; goldTabs?: boolean; activeGoldTab?: GoldTab; onGoldTabChange?: (tab: GoldTab) => void; intrinsicGoldPrice?: number | null; sortSources?: boolean }) {
  const liveCount = market.sources.filter((source) => source.status === "live" && source.price !== null).length;
  const displaySources = sortSources ? [...market.sources].sort((a, b) => (b.price ?? -1) - (a.price ?? -1)) : market.sources;
  ...
  return <section className={`market-panel ${tone}`} aria-labelledby={`${tone}-title`}>
    <div className="market-topline"><div><p className="market-kicker">{titleEnglish}</p>{titleControl}</div><span className="market-symbol" aria-hidden="true">{tone === "gold" ? "Au" : tone === "coin" ? "C" : "$"}</span></div>
    <div className="market-average"><div><span>{summaryLabel}</span><strong>{summaryValue ?? amount(market.averagePrice, unit)}</strong></div><span className="source-count">{liveCount} از {market.sources.length} منبع فعال</span></div>
    <div className="source-list" role="list">{displaySources.map((source, index) => {
...
export default function Home() {
  const [data, setData] = useState<PriceResponse | null>(null); const [loading, setLoading] = useState(true); const [error, setError] = useState(false); const [remaining, setRemaining] = useState(REFRESH_SECONDS); const [theme, setTheme] = useState<"light" | "dark">("light"); const [activeTab, setActiveTab] = useState<QuoteTab>("usd"); const [activeGoldTab, setActiveGoldTab] = useState<GoldTab>("gold");
  const priceRequests = useRef<Partial<Record<GoldTab | QuoteTab, AbortController>>>({});
  const fetchPrices = useCallback(async (market: GoldTab | QuoteTab, signal?: AbortSignal) => {
    ...
    try {
      ...
      while (true) {
        ...
        for (const message of messages) {
          ...
          const payload = JSON.parse(json) as { market?: GoldTab | QuoteTab; source?: PriceSource; goldOunce?: number | null; updatedAt: string };
  ...
  useEffect(() => { void fetchPrices("coin"); const interval = window.setInterval(() => void fetchPrices("coin"), REFRESH_SECONDS * 1000); return () => window.clearInterval(interval); }, [fetchPrices]);
