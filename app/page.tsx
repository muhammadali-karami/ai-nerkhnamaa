...
function MarketPanel({ market, title, titleEnglish, unit, tone, tabs, activeTab, onTabChange, preserveSourceOrder }: { market: Market; title: string; titleEnglish: string; unit: string; tone: "gold" | "dollar"; tabs?: boolean; activeTab?: QuoteTab; onTabChange?: (tab: QuoteTab) => void; preserveSourceOrder?: boolean }) {
  const liveCount = market.sources.filter((source) => source.status === "live" && source.price !== null).length;
  const sortedSources = preserveSourceOrder ? market.sources : [...market.sources].sort((a, b) => (b.price ?? -1) - (a.price ?? -1));
