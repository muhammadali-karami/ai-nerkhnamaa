...
const state = { active: "gold", markets: { gold: [], usd: [], usdt: [] }, updatedAt: null, theme: "light", request: null };
...
function render() { const sources = state.markets[state.active]; const marketNames = { gold: "طلای ۱۸ عیار", usd: "دلار آمریکا", usdt: "تتر" }; const unit = state.active === "gold" ? "تومان / گرم" : "تومان"; const value = state.active === "usd" ? highest(sources) : average(sources); $("label").textContent = state.active === "usd" ? "بالاترین نرخ صرافی" : marketNames[state.active]; $("average").textContent = value === null ? "—" : `${number(value)} ${unit}`; $("updated").textContent = state.updatedAt ? `بروزرسانی: ${new Intl.DateTimeFormat("fa-IR", { hour: "2-digit", minute: "2-digit" }).format(state.updatedAt)}` : "در حال دریافت اطلاعات…"; $("sources").innerHTML = [...sources].sort((a, b) => (b.price ?? -1) - (a.price ?? -1)).map((source, index) => `<article class="source ${source.price ? "" : "offline"}"><span class="rank">${index + 1}</span><div><strong>${source.name}</strong><p>${source.price ? "فعال" : "ناموجود"}</p></div><p class="price">${source.price ? `${number(source.price)} ${unit}` : "قیمت در دسترس نیست"}</p></article>`).join(""); }
async function refresh() { state.request?.abort(); const request = new AbortController(); state.request = request; $("refresh").disabled = true; $("notice").hidden = true; try { const [gold, usd, usdt] = await Promise.all([goldSources(request.signal), usdSources(request.signal), tetherSources(request.signal)]); if (state.request !== request) return; state.markets = { gold, usd, usdt }; state.updatedAt = new Date(); render(); } catch (error) { if (error.name !== "AbortError") { $("notice").textContent = "دریافت اطلاعات فعلاً ممکن نیست؛ دوباره تلاش کنید."; $("notice").hidden = false; } } finally { if (state.request === request) { state.request = null; $("refresh").disabled = false; } } }
